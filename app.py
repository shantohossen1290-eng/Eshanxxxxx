...
def _gen_captcha_image() -> Tuple[Optional[bytes], str, List[str]]:
    """Generate a very clear CAPTCHA image and its answer options.

    Four large, high-contrast characters are shown. Exactly one character
    has a thick red circle around it.  The circle is positioned from the
    actual text bounding box so it cannot miss the character.
    """
    ...
    # Very light background lines/noise, kept intentionally subtle.
    for _ in range(6):
        y = random.randint(30, H - 70)
        draw.line([(20, y), (W - 20, y + random.randint(-18, 18))],
                  fill=(225, 230, 235), width=2)
    for _ in range(120):
        x, y = random.randint(0, W - 1), random.randint(0, H - 55)
        draw.point((x, y), fill=(220, 225, 230))

    font = _captcha_font(145)
    centers = []
    slot_w = W // 4

    for i, ch in enumerate(text):
        # Use textbbox to place each glyph precisely in its slot.
        try:
            bbox = draw.textbbox((0, 0), ch, font=font, stroke_width=2)
            tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
            tx = i * slot_w + (slot_w - tw) // 2 - bbox[0]
            ty = 100 - bbox[1]
        except Exception:
            tw, th = 90, 120
            tx = i * slot_w + (slot_w - tw) // 2
            ty = 100

        # Strong black character with a tiny outline for mobile readability.
        draw.text((tx, ty), ch, font=font, fill=(10, 15, 25),
                  stroke_width=2, stroke_fill=(10, 15, 25))
        centers.append((tx + tw // 2, ty + th // 2))

    # Thick red circle centered on the actual glyph center.
    cx, cy = centers[correct_idx]
    r = 78
    for width in (10, 8, 6):
        draw.ellipse([cx-r, cy-r, cx+r, cy+r],
                     outline=(220, 25, 35), width=width)

    # Clear instruction strip.
    strip_y = H - 58
    draw.rectangle([0, strip_y, W, H], fill=(235, 239, 244))
    hint_font = _captcha_font(26)
    hint = "TAP THE CIRCLED CHARACTER"
    try:
        bbox = draw.textbbox((0, 0), hint, font=hint_font)
        tw = bbox[2] - bbox[0]
        draw.text(((W - tw) // 2, strip_y + 14), hint,
                  font=hint_font, fill=(30, 35, 45))
    except Exception:
        pass
